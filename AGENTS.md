...
## Układ katalogu raportu HTML

Każdy raport HTML ma własny katalog. W jego głównym poziomie umieszczaj wyłącznie `raport.html` oraz podkatalog `materialy/`. Dotyczy to generatora kreacji i analiz HTML opracowywanych przez agenta. Dane JSON, obrazy, filmy, style, skrypty, źródłowy Markdown, PDF towarzyszący temu raportowi, notatki analityczne i kontrolę `language-review.json` zapisuj w `materialy/` (możesz tworzyć w nim dalsze podkatalogi). Nie pozostawiaj obok HTML dodatkowych plików. Odnośniki HTML mają być względne, np. `materialy/media/obraz.png`, aby cały katalog można było przenieść. Przekazuj użytkownikowi link do `raport.html`. Nową wersję raportu twórz w osobnym katalogu; wersje nie są dodatkowymi plikami HTML obok siebie.

Kontrola językowa w `materialy/language-review.json` wskazuje rzeczywistą ścieżkę i hash sprawdzonego pliku. Przed przekazaniem sprawdź kompletność katalogu oraz lokalne odnośniki do materiałów.
...
4. Dla zapisywanego raportu odnotuj kontrolę w osobnym pliku `language-review.json` (dla raportu HTML w `materialy/`, dla pozostałych raportów obok niego): ścieżkę i SHA-256 sprawdzonej treści, commit skilla z `skills/miodkuj/UPSTREAM.json`, datę oraz wynik `PASS` lub `FLAG`. Zapisuj tylko faktycznie wykonaną kontrolę. Nie dodawaj tych pól do kontraktu raportu silnika. Przy samej odpowiedzi w rozmowie nie trzeba tworzyć pliku.
